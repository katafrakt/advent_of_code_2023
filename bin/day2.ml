...
let get_color_num list_of_tuples color : int =
  match
    List.find_opt (fun (_, col) -> String.equal col color) list_of_tuples
  with
  | Some (n, _) -> n
  | _ -> 0
...
let parse_result res_string : result =
  let tuples =
    res_string |> String.split_on_char ',' |> List.map String.trim
    |> List.map (split_in_two ' ')
    |> List.map (fun (n, col) -> (int_of_string n, col))
  in
...
let game_power game =
  let min_green =
    game.results |> List.map (fun x -> x.green) |> max_number_list
  in
  let min_red = game.results |> List.map (fun x -> x.red) |> max_number_list in
  let min_blue =
    game.results |> List.map (fun x -> x.blue) |> max_number_list
  in
  min_green * min_red * min_blue

let run () =
  let games = file |> Advent.read_lines |> List.map parse_game in
  ...
  let powers_sum = games |> List.map game_power |> List.fold_left ( + ) 0 in
