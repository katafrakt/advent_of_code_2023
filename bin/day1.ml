...
let find_nums line =
  ...
  match (first, last) with Some x, Some y -> (x * 10) + y | _ -> 0

let part1 () =
  file |> Advent.read_lines
  |> List.map string_to_char_list
  |> List.map find_nums |> List.fold_left ( + ) 0 |> string_of_int
  |> print_endline
...
let rec replace_words str pos =
  if pos < String.length str then
    let new_str =
      ...
      |> maybe_replace_num pos (Str.regexp {|nine|}) "9ine"
    in
    if new_str != str then replace_words new_str 0
    else replace_words str (pos + 1)
  else str

let replace_words str = replace_words str 0

let part2 () =
  file |> Advent.read_lines |> List.map replace_words
  |> List.map string_to_char_list
  |> List.map find_nums |> List.fold_left ( + ) 0 |> string_of_int
  |> print_endline
