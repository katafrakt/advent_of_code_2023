...
let write_seeds seeds =
  List.iter (fun x -> print_int x; print_char ' ') seeds
...
let rec read_transformations transformations current_opt lines =
    let current = match current_opt with
  ...
  | line :: rest ->
    (match String.split_on_char ' ' line with
      | [drs ; srs ; length] ->
        let transformation = {drs = int_of_string drs ; srs = int_of_string srs ; length = int_of_string length} in
        read_transformations transformations (Some (transformation :: current)) rest
...
let transform_one acc transformation =
  List.map (fun seed ->
    match List.find_opt (fun t -> t.srs <= seed && seed - t.srs < t.length) transformation with
